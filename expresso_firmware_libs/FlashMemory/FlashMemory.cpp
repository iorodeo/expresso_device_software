...
void FlashMemory::printBasePageAddress() {
    ...
    for (uint8 i=0; i<numPages; i++) {
        Serial << "page: " << i << " " << _HEX(basePageAddress[i]) << endl;
    }
    Serial << endl;
}
...
void FlashMemory::printPage(uint8 pageNum, bool asUint8) {
    ...
    uint8 value1;
    if (pageNum >= numPages) {
        Serial << "pageNum > numPages" << endl;
        return;
    }
    Serial << "page, address, value" << endl;
    for (uint16 address=0; address<maxAddress; address++) { 
        if (asUint8) {
            readData(pageNum, address, value0, value1);
            Serial << pageNum << ", " << address << ", " << value0 << ", " << value1 << endl; 
        }
        else {
            readData(pageNum, address, value);
            Serial << pageNum << ", " << address << ", " << value << endl; 
        }
    }
    Serial << endl;
}
